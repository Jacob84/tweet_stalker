...
RSpec.describe UserListsTracker do
  ...
  before do
    ...
    @user_lists_tracker = UserListsTracker.new(@lists_retriever)
  end
  ...
  describe 'when getting backed tracked lists' do
    ...
    context 'with backed lists' do
      it 'returns suitable lists with tracking' do
        TrackedList.create(:user_id => 1, :twitter_list_id => 111111, :tracked => true)

        result = @user_lists_tracker.get_lists(1)

        expect(result.length).to be 2
        assert_tracked_list(result[0], 321139, 'name', 11, 22, 'public', false)
        assert_tracked_list(result[1], 111111, 'name', 11, 22, 'public', true)
      end
    end
  end
